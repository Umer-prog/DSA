...
// for other graph
void DeleteEdge2(char source, char destination) { //deleting an edge from a specific vertex
...
void Prims(Vertex* start) {
	...
	int check = 0;
	while (true) {
		// for all connected function
		int* Visit = new int[VertexCount];
		for (int j = 0; j < VertexCount; j++) {
			Visit[j] = 0;
		}
		AllConn(Cstart2, Visit);// telling which vertex are visited
		// checking function output (debug)
		...
		check = 0;
		// if all vertexes visited
		for (int k = 0; k < VertexCount; k++) {
			if (Visit[k] == 0)check = 1;
		}
		// check =0  when all visited so the min spanning tree is made so exit
		if (check == 0)break;
		int lowest = 99999999999999;
		...
		for (int i = 0; i < n + 1; i++) {// to access all visited vertex
			...
			while (sub2 != NULL) { // accessing all the sublist of vertex
				//checks every sublist for lowest values and store them in temp to check with further list for comparison
		...
		if (check == 1) {
			...
			//checks if the edge created creats a loop
			if (LoopDetect(save->source->vertex) == 1) { // Checking and deleting if the inserted edge made a loop
				// deletes from our new graph
				DeleteEdge2(save->source->vertex, save->desti->vertex);
				
				//also deletes  from previous graph so its not considered next time
				DeleteEdge(save->source->vertex, save->desti->vertex);
				cout << "deleted" << save->source->vertex << "\t" << save->desti->vertex;
				continue;
			}
			//if visited update both the arrays 
			n++;
